...
public class LocalSearchMin {

    public AbstractAssignmentProblem problem; // конкретная задача на МАКСИМУМ
    ...
    public int maxRecPi = -1; // минимальное значение функции

    // конструктор поиска
    public LocalSearchMin(AbstractAssignmentProblem problem) {
    ...
    public void generateRandomPi(){
        ...
        updateRec();
        if ( problem.function(pi) > maxRecPi) { maxRecPi = problem.function(pi); }
    }
    ...
    public double getErrorRate(){
        ...
        return 100*(rec - problem.fPi)/(double)problem.fPi;
